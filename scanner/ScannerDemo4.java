...
* Purpose: this example will introduce us to the "Scanner Bug", which is not
		   ...
		   REVISION: here we show how to do a MANUAL FLUSH of the KEYBOARD BUFFER
		   to get rid of the EOL character left behind by that RUDE HOUSE GUEST
		   method called next().
...
*/
import java.util.Scanner;//we're telling the interpreter where to find the
...
public class ScannerDemo4
{
	public static void main(String[]args)
	{
		...
		//REVISION:Step 3A: here we will do a manual buffer flush because we
		// need to use the nextLine() method down below to read in town
		//ALL YOU NEED TO DO IS JUST CALL nextLine() by itself to clear the buffer
		
		input.nextLine(); //this FLUSHES the EOL character out of the buffer.
